...
class ChunkRateInfo(NamedTuple):
	...
	def __str__(self) -> str:
		return f'{self.element_name}: Number {self.total_elements} Avg rate {to_si_numeric_unit(self.total_elements/self.total_memory_rate.elapsed_time_sec, f"/s")} :: Mem rate: Avg {self.total_memory_rate!s} Split {self.rolling_memory_rate!s} :: Time: Elapsed {elapsed_time_str(self.total_memory_rate.elapsed_time_sec)} Split {elapsed_time_str(self.rolling_memory_rate.elapsed_time_sec)}'
