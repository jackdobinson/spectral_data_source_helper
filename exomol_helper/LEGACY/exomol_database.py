...
from .cfg.const import (
	EXOMOL_API_URL_FMT,
	EXOMOL_DATABASE_URL,
	EXOMOL_DATABASE_ROOT_URL
)

#from ..datatypes.exomol.gas_descriptor import ExomolGasDescriptor
from .filetypes.root import ExomolRootFormat
from .filetypes.isotope_def import ExomolIsotopeDef
#from .filetypes.states import ExomolIsotopeStates
#from .filetypes.trans import ExomolIsotopeTrans
...
def iter_exomol_api_data(exomol_root, directory : str):
	for mi in exomol_root.molecule_info:
		...
		api_data = Cache.EXOMOL_API_DATA.get(mol_formula, None)
		...
		# store in cache
		Cache.EXOMOL_API_DATA[mol_formula] = api_data
...
class EXOMOL_DB:
	...
	all_file : str = EXOMOL_DATABASE_ROOT_URL # NOTE: There is a JSON version at https://www.exomol.com/db/exomol.all.json
	...
	def db_load_gasses(cur):
		...
		for mol_id, gas_props in gas_data.gas_info.items():
			...
			EXOMOL_DB.db_add_molecule(cur, gas_props['name'])
			
			# Add a gas entry for each molecule
			EXOMOL_DB.db_add_gas(cur, gas_props['name'])
			...
			for i, (iso_id, iso_props) in enumerate(gas_props['isotope'].items()):
				...
				EXOMOL_DB.db_add_gas(
					cur,
					norm_iso_name,
				)
				
				# Single isotopologue gasses are only composed of themselves
				EXOMOL_DB.db_add_isotopologue_gas_component(
					cur,
					norm_iso_name,
					int(mol_id),
					int(iso_id),
			...
			EXOMOL_DB.db_add_molecular_gas_components(
				cur,
				gas_props['name'],
				[(gas_props['name'], 1.0)],
				True
		...
		for extra_gas_name, extra_gas_info in extra_gasses.items():
			EXOMOL_DB.db_add_gas(cur, extra_gas_name)
			EXOMOL_DB.db_add_molecular_gas_components(
				cur,
				extra_gas_name,
